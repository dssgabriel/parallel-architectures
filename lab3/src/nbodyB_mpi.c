                      ...
                      const int particles)
{
    ...
    for (size_t t = start_particle; t < end_particle; t += TILE_SIZE) {
        f32 fx[TILE_SIZE] = { 0.0f };
        f32 fy[TILE_SIZE] = { 0.0f };
        f32 fz[TILE_SIZE] = { 0.0f };
...
int main(int argc, char **argv)
{
    ...
    particles_t p;
    p.px = _mm_malloc(nb_particles * sizeof(f32), 64);
    p.py = _mm_malloc(nb_particles * sizeof(f32), 64);
    p.pz = _mm_malloc(nb_particles * sizeof(f32), 64);
    p.vx = _mm_malloc(nb_particles * sizeof(f32), 64);
    p.vy = _mm_malloc(nb_particles * sizeof(f32), 64);
    p.vz = _mm_malloc(nb_particles * sizeof(f32), 64);

    srand(0);
    for (size_t i = 0; i < nb_particles; i++) {
        u64 r1 = rand(), r2 = rand();
        f64 sign = (r1 > r2) ? 1.0f : -1.0f;
