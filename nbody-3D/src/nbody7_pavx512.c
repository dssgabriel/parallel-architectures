...
static inline f32 hsum(const f512 x)
{
...
particles_t* particles_new(const u64 nb_bodies)
{
    ...
    p->px = aligned_alloc(32, nb_bodies * sizeof(f32));
    p->py = aligned_alloc(32, nb_bodies * sizeof(f32));
    p->pz = aligned_alloc(32, nb_bodies * sizeof(f32));
    p->vx = aligned_alloc(32, nb_bodies * sizeof(f32));
    p->vy = aligned_alloc(32, nb_bodies * sizeof(f32));
    p->vz = aligned_alloc(32, nb_bodies * sizeof(f32));
...
#endif
            const f512 invd_3 =
                _mm512_mul_ps(_mm512_mul_ps(invd, invd), invd); // 13
...
void particles_print(particles_t* p, const config_t cfg)
{
    ...
    fprintf(fp, "%llu\t%u\t%lf\t%s\n\n", cfg.nb_bodies, cfg.nb_iter, cfg.dt,
            precision);
...
int main(int argc, char** argv)
{
    ...
    if (!p)
        return fprintf(
                   stderr,
                   "\033[1;31merror:\033[0m failed to allocate particles\n"),
               1;
    ...
    for (usize i = 0; i < cfg.nb_iter; i++) {
        ...
        // GFLOPS
        const f64 h2 = (19.0f * h1 + 30.0f * (f64)(cfg.nb_bodies)) * 1e-9;
        ...
        fprintf(stderr, "%5zu %10.3e %10.3e %8.1f %s\n", i + 1, (end - start),
                h1 / (end - start), h2 / (end - start),
                (i < cfg.nb_warmups) ? "*" : "");
        fflush(stderr);
    }
